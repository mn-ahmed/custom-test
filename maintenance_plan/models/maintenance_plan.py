...
class MaintenancePlan(models.Model):
    ...
    equipment_id = fields.Many2one(string='Equipment',
                                   ...
                                   ondelete='cascade')
    maintenance_kind_id = fields.Many2one(string='Maintenance Type',
    ...
    @api.depends('interval', 'interval_step', 'maintenance_kind_id',
    ...
    def unlink(self):
        """ Restrict deletion of maintenance plan should there be maintenance
            requests of this type which are not done for its equipment """
        for plan in self:
            ...
            if request:
                raise UserError(_('The maintenance plan %s of equipment %s '
                                  ...
                                  'done, remove its maintenance type or '
                                  'delete it first.') % (
    ...
    _sql_constraints = [
        ('equipment_kind_uniq', 'unique (equipment_id, maintenance_kind_id)',
         "You cannot define multiple times the same maintenance type on an "
